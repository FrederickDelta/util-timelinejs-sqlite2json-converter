...
function convert(inputSqlite, outputJSON){
  let timelineObj
  let eventsObj = []
  let titleObj
  let erasObj = []

  let db = new sqlite3.Database(inputSqlite, sqlite3.OPEN_READONLY, (err) => {
    if (err) {
  ...
  db.all('SELECT * FROM slides ORDER BY start_year ASC', (err, rows) => {
    ...
    fs.writeFileSync(outputJSON, JSON.stringify(timelineObj))
